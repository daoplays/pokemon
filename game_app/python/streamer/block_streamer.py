...
if (last_db_row != None):
    ...
    latest_block = get_slot(dev_client)
...
print("Starting with row: ", current_row_id_to_insert, " Current block: ", current_block, latest_block, latest_block - current_block)
while(True):

    # get all the blocks after and including current_block
    block_list = get_block_list(quick_node_dev, current_block)
    ...
    if(len(block_list) == 1):
        ...
        b_idx, data = get_data_from_block(block_list[0], blocks[block_list[0]]["transactions"])
        ...
        with cf.ThreadPoolExecutor(len(block_list)) as executor:
            futures = [executor.submit(get_data_from_block, block_id, blocks[block_id]["transactions"]) for block_id in block_list]
